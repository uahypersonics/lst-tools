...
def cmd_clean_parsing(
    ...
    """Remove solver output artifacts generated by ``lst-tools setup parsing``.

    Removes solver output files (Frequency*, SpectralGrid.dat, fort.*, etc.)
    from the target directory.  The input deck (``lst_input.dat``), run scripts
    (``run.*.*``), and ``run.log`` are kept.
    """
